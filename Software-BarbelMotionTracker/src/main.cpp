...
struct Measurement_t {
  double lin_acc_x, lin_acc_y, lin_acc_z;                // BNO055               | linear acceleration, double in m/s^2
  double abs_quat_w, abs_quat_x, abs_quat_y, abs_quat_z; // BNO055               | absolute orientation, double quaternion
  float acc_x, acc_y, acc_z;                             // ICM 42688P           | acceleration, float in m/s^2
  float rot_x, rot_y, rot_z;                             // ICM 42688P           | angular velocity, float in dps
  float temp;                                            // ICM 42688P not used  | temperature gyro die, float in C
  double mag_x, mag_y, mag_z;                            // MCC5983MA            | double in uT (+-800uT)
  float abs_height;                                      // BMP390               | altitude, float in meters
  int64_t time_since_last_measure_us;                    //                      | int in micro seconds
};
